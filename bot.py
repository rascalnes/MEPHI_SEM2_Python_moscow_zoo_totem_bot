...
async def send_next_question(message, context, user_id):
    """Отправка следующего вопроса новым сообщением"""
    user_data = user_data_store[user_id]
    q_index = user_data["current_question"]
...
async def calculate_and_show_result(update, context, user_id):
    ...
    if hasattr(update, 'callback_query') and update.callback_query:
        ...
        # Если нет ни того, ни другого, выходим
        logger.error(f"Cannot send result to user {user_id}: no message target")
        return

    # Отправка изображения животного
    image_path = os.path.join("images", totem['image'])
    ...
    try:
        if os.path.exists(image_path):
            with open(image_path, 'rb') as photo:
                ...
                logger.info(f"Photo sent to user {user_id}: {image_path}")
        else:
            ...
            if os.path.exists("images"):
                logger.info(f"Files in images: {os.listdir('images')}")
            else:
                logger.warning("Folder 'images' does not exist!")
...
async def handle_result_action(update: Update, context):
    ...
    if action == "guardianship":
        ...
        # Возврат к результату (нужно показать результат снова)
        user_data["quiz_active"] = True
        await calculate_and_show_result(query, context, user_id)
...
async def handle_feedback(update: Update, context):
    ...
    await update.message.reply_text(
        ...
        "Хотите пройти викторину? Нажмите *🎮 Начать викторину*!",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_main_keyboard()
    )
...
async def contact_manager(update: Update, context):
    ...
    result = user_data.get("result", "не проходил(а) викторину")
    ...
    contact_text = (
        ...
        f"📝 *Ваш результат викторины:* {result}\n\n"
    ...
    if update and update.effective_chat:
        try:
            await update.effective_chat.send_message(
                ...
                "Если ошибка повторяется, используйте команду /start для перезапуска."
            )
        except:
            pass
